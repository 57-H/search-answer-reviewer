...
当前仓库是 Beta 候选。一次 [Codex 100 批开发实验](docs/codex-coverage-2026-09-23.md)得到 97/100 的结构化审查覆盖率代理值，漏审 3 批。由于 Codex 原始轨迹没有暴露结果首次使用事件，且人工核定的语义对照实验尚未完成，这个数字不是正式发布指标，项目仍不宣称稳定触发率或降低幻觉的百分比。评测方式见[评测协议](eval/protocol.md)。
