...
Two repetitions of that case were then run from the public archive for revision `63bc48ecc925280cb217545463e2baef1242cb12` (`c11fab84005449979c8c2ffbd78651ecf590da476761406cc9d3166b1390be5c`). Neither met review-before-use coverage:

- In the first repetition, the model attempted to read both the general research skill and Search Answer Reviewer in one chained command. The first path was invalid, so shell short-circuiting prevented the Search Answer Reviewer file from being read. A subsequent path search also missed the installed file. The model then searched and answered without a review record or status.
- In the second repetition, the skill loaded and the initial batch passed as `已审查 29/29`. A supplementary record failed because a `supported` claim retained an unresolved gap. The model did not perform the required repair-and-retry step and still used supplementary facts in the answer.

These failures occur at different layers: skill loading and post-validation recovery. They show that stronger ordinary-Skill instructions alone do not enforce the gate. Further prompt additions are paused; the public Beta remains blocked pending either substantially better measured behavior or an execution environment that can intercept result use mechanically.
